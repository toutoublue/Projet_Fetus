...
function readOFF_from_string(string, meshArray)
{
    ...
    var colorL = [[0.3,0.9,0.6],[0.7,0.2,1],[0.6,0.2,0.7],[0.9,0.5,0.5]];
    
    for (var j = 0; j < num; j++) {
        var meshtmp = new Mesh();
        meshtmp.V = new Array();
        meshtmp.N = new Array();
        meshtmp.TC = new Array();
        meshtmp.F = new Array();
        meshtmp.FN = new Array();
        meshtmp.FTC = new Array();
        meshtmp.color = [];
        
        var size = lines[(1 + NumofVertices[j])].trim().split(" ");
        var sizeV = size[0], sizeT = size[1], tmp = size[2];
        
        for (var i=0;i<sizeV*4;i+= 4){
            meshtmp.color[i] = colorL[j][0];
            meshtmp.color[i+1] = colorL[j][1];
            meshtmp.color[i+2] = colorL[j][2];
			switch(j){
			case 0: meshtmp.color[i+3] = 0.5;break;
			case 1: meshtmp.color[i+3] = 1;break;
			case 2: meshtmp.color[i+3] = 1;break;
			case 3: meshtmp.color[i+3] = 0.3;break;
			}
        }
        ...
        for (var l = 2 + NumofVertices[j]; l < 2 + parseInt(sizeV) + NumofVertices[j]; l++) {
            ...
            if (data.length != 3)
            {
                ...
                meshtmp.V.push.apply(meshtmp.V, data.map(Number));
            }
        }
        ...
        for (var l = 2 + parseInt(sizeV) + NumofVertices[j]; l < 2 + parseInt(sizeV) + parseInt(sizeT) + NumofVertices[j]; l++) {
            ...
            if (parseInt(NumOfVertices) != 3)
            {
                ...
                meshtmp.F.push.apply(meshtmp.F, data.map(Number));
            }
        }
        meshArray.push(meshtmp);
    }

...
function handleLoadedMesh(mesh)
{
    ...
    for (var v = 0; v < VBOs.length; v++)
    {
        ...
        vbo.ColorBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, vbo.ColorBuffer);
        var colors = mesh.color;
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);
        vbo.ColorBuffer.itemSize = 4;
        vbo.ColorBuffer.numItems = vbo.V.length ;  
    }
}
var teapotAngle = 180;
...
function drawScene() {
    ...
    for (var v = 0; v < VBOs.length; v++)
    {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, vbo.ColorBuffer);
        gl.vertexAttribPointer(shaderProgram.vertexColorAttribute, vbo.ColorBuffer.itemSize, gl.FLOAT, false, 0, 0);
...
function handleFiles(files)
{
    ...
    if (file)
    {
        ...
        {
            setTimeout(function() {
                readOFF_from_string(evt.target.result, meshArray);
                setTimeout(function() {
